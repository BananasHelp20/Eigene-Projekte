...
/** ACTUAL MAIN, THIS IS THE ACTUAL MAIN, THE FIRST ONE */
    ...
    webServer.on("/increaseSecPump1", HTTP_GET, [](AsyncWebServerRequest *request)
                 {
                        if (durationPump1 < 600) {
                           ...
                           Serial.println("flow duration: " + String(durationPump1));
                        } else {
...
void loop()
{
    if (running)
    {
        ...
        { // NOTE AN MICH, in der While is der dann de ganze zeit!
            ...
            preferences.putInt("dayGoal2", daysToPump2);
            preferences.putInt("dayGoal3", daysToPump3);
            timeCtr1 = (currentTimeInSeconds - lastTimeAnActionTookPlaceInSeconds1);
            timeCtr2 = (currentTimeInSeconds - lastTimeAnActionTookPlaceInSeconds2);
            timeCtr3 = (currentTimeInSeconds - lastTimeAnActionTookPlaceInSeconds3);
        ...
        while (!running) {
            Serial.println("Not doin anything today");
            delay(1000);
        }
        delay(1000);
    }
